...
#define ITU_DITHER      0x10    ///< This surface need to be dithered.
	...
	*/
	...
	void ituFrameFuncDrawJpeg(int gui_x, int gui_y, int gui_w, int gui_h, char *jpg_path);
	void ituFrameFuncFlip(int surf_x, int surf_y);
...
#define ITU_TABLEGRID_SINGLE_SELECT    0x1    ///< Select a item only on selection.
	...
	*/
	typedef struct
	{
		...
		char path[4096];                                        ///< Loaded file path
		ITUVariable variables[ITU_VARIABLE_SIZE];                   ///< Variables
		int threadID;                                         ///< The current thread ID
