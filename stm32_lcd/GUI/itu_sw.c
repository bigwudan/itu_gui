...
{
    ITUSurface *surf = (ITUSurface *)calloc(1, sizeof(ITUSurface));
    if (!surf)
    {
...
#endif // !_WIN32
    {
		...
		surf->addr = (uint32_t)calloc(1, size);
...
static void SWTransform(ITUSurface* dest, int dx, int dy, int dw, int dh, ITUSurface* src, int sx, int sy, int sw, int sh, int cx, int cy, float scaleWidth, float scaleHeight, float angle, ITUTileMode tilemode, bool enableAlpha, bool enableConstantAlpha, uint8_t constantAlphaValue)
{
    if (tilemode == ITU_TILE_REFLECT)
    {
        for (int i = 0; i < sw; i++)
        {
            SWBitBlt(dest, dx + i, dy, 1, dh, src, sw - i - 1, sy);
        }
    }
}
...
//		LCD_Color_Fill(0,0,200,100,(u16 *)swLcdAddr);
